...
const initialForm = {
  name: '', description: '', entryFee: 50, maxParticipants: 8, prizeCoins: 500,
  cosmeticRewards: [
    { rank: 1, type: 'skin', itemId: '' },
    { rank: 2, type: 'emote', itemId: '' },
    { rank: 3, type: 'avatar_frame', itemId: '' },
  ],
  startTime: '', registrationClosesAt: '', reason: '',
};
    ...
    <div className="flex flex-col gap-5">
      ...
      {canWrite && <AdminCard className="p-4">
        ...
        <form onSubmit={createTournament} className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          ...
          <Field label="Sức chứa (format cố định)"><input className={inputClass} type="number" value="8" readOnly /></Field>
          ...
          {form.cosmeticRewards.map((reward, index) => <Field key={reward.rank} label={`Vật phẩm độc quyền hạng ${reward.rank}`}><div className="flex gap-2"><select className={`${inputClass} w-36`} value={reward.type} onChange={(e) => setForm({ ...form, cosmeticRewards: form.cosmeticRewards.map((item, itemIndex) => itemIndex === index ? { ...item, type: e.target.value } : item) })}><option value="skin">Skin</option><option value="emote">Emote</option><option value="avatar_frame">Khung avatar</option></select><input className={inputClass} value={reward.itemId} onChange={(e) => setForm({ ...form, cosmeticRewards: form.cosmeticRewards.map((item, itemIndex) => itemIndex === index ? { ...item, itemId: e.target.value } : item) })} placeholder="Item ID (để trống nếu không thưởng)" /></div></Field>)}
      ...
      <ConfirmDialog open={!!payoutPreview} title="Xác nhận payout giải đấu" description={`Sẽ cộng ${payoutPreview?.totals?.coins || 0} Coin và cấp vật phẩm trực tiếp cho ${payoutPreview?.rows?.length || 0} người. Preview hết hạn lúc ${formatDate(payoutPreview?.expiresAt)}.`} confirmLabel="Thực hiện payout" confirmDisabled={!!busy || confirmationUsername.trim() !== adminUsername} onConfirm={executePayout} onClose={() => { setPayoutPreview(null); setConfirmationUsername(''); }}>
        <div className="grid gap-3"><Field label={`Nhập username để xác nhận: ${adminUsername}`}><input className={inputClass} value={confirmationUsername} onChange={(e) => setConfirmationUsername(e.target.value)} autoComplete="off" /></Field><div className="max-h-40 overflow-auto border border-[var(--admin-border)] bg-[var(--admin-surface)] p-2 text-sm">{payoutPreview?.rows?.map((row) => <p key={row.participantId} className="font-mono">#{row.rank} {row.username}: +{row.coins} Coin{row.cosmetics?.length ? ` · ${row.cosmetics.map((item) => item.itemId).join(', ')}` : ''}</p>)}</div></div>
      </ConfirmDialog>
    </div>
  );
}
...
function TournamentDetail({ detail, canWrite, canPayout, reason, setReason, participantId, setParticipantId, registerParticipant, updateScore, nextTransitions, setConfirmAction, previewPayout, payoutPreview, busy }) {
  ...
  return <div className="grid gap-5">
    <AdminCard className="p-4">
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-4">{[['Người chơi', `${tournament.registeredCount}/${tournament.maxParticipants}`], ['Entry fee', `${tournament.entryFee} Coin`], ['Prize', `${tournament.prizePool?.coins || 0} Coin`], ['State version', tournament.stateVersion]].map(([label, value]) => <div key={label} className="rounded-lg border border-[var(--admin-border)] bg-[var(--admin-surface-muted)] p-3"><p className="text-xs font-semibold uppercase text-[var(--admin-text-muted)]">{label}</p><p className="mt-1 font-mono font-semibold">{value}</p></div>)}</div>
      {!!tournament.cosmeticRewards?.length && <p className="mt-3 text-sm font-bold">Vật phẩm độc quyền: {tournament.cosmeticRewards.map((reward) => `#${reward.rank} ${reward.itemId}`).join(' · ')}</p>}
    ...
    <AdminCard className="p-4"><h3 className="font-sans font-semibold">Người tham gia</h3>{participants.length === 0 ? <div className="mt-3"><EmptyState title="Chưa có người đăng ký" /></div> : <div className="mt-3 overflow-auto"><table className="w-full min-w-[600px] border-collapse text-sm"><thead><tr className="bg-black text-left text-white"><th className="p-3">Người chơi</th><th className="p-3">Thanh toán</th><th className="p-3">Điểm / override</th><th className="p-3">Hạng</th><th className="p-3">Payout</th></tr></thead><tbody>{participants.map((participant) => <ParticipantRow key={participant._id} participant={participant} active={tournament.status === 'active'} canWrite={canWrite} updateScore={updateScore} />)}</tbody></table></div>}</AdminCard>

    <AdminCard className="p-4"><h3 className="font-sans font-semibold">Lịch thi đấu tự động</h3>{!tournament.bracket?.rounds?.length ? <div className="mt-3"><EmptyState title="Lịch chưa được tạo" description="Hai bảng và vòng chung kết được tạo tự động khi đủ 8 người và bắt đầu giải." /></div> : <div className="mt-4 grid gap-4 md:grid-cols-2">{tournament.bracket.rounds.flatMap((round) => round.matches.map((match) => <article key={match.id} className="rounded-lg border border-[var(--admin-border)] bg-[var(--admin-surface-muted)] p-3"><div className="flex justify-between gap-2"><p className="font-mono text-xs font-semibold uppercase">{round.name} · {match.id}</p><StatusBadge tone={match.status === 'completed' ? 'success' : match.status === 'pending' ? 'warning' : 'neutral'}>{match.status}</StatusBadge></div>{match.roomCode && <p className="mt-1 font-mono text-xs">Room: {match.roomCode}</p>}<div className="mt-2 grid gap-2">{match.participants.map((person) => { const result = match.result?.find((row) => row.participantId === person.participantId); return <div key={person.participantId} className="rounded-lg border border-[var(--admin-border)] bg-[var(--admin-surface)] p-2 font-bold">{person.username}{result ? ` · #${result.placement} · ${result.points}đ` : ''}</div>; })}</div></article>))}</div>}</AdminCard>
  </div>;
}
