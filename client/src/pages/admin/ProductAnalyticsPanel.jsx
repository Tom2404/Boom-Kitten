...
export default function ProductAnalyticsPanel() {
  ...
  return <div className="flex flex-col gap-5">
    <SectionHeader title="Product Analytics" description="Funnel, retention, kinh tế Coin và content performance dùng một khoảng thời gian UTC nhất quán." actions={<Button onClick={load}>Làm mới</Button>} />
    ...
    {loading ? <AdminCard className="p-4"><SkeletonBlock rows={8} /></AdminCard> : data && <>
      ...
      <AdminCard className="p-4"><h3 className="font-sans font-semibold">Coin source / sink</h3><div className="mt-4 max-w-md"><article className="rounded-lg border border-[var(--admin-border)] bg-[var(--admin-surface-muted)] p-4"><p className="font-sans font-semibold uppercase">Coin</p><dl className="mt-3 grid gap-2 text-sm"><div className="flex justify-between"><dt>Source</dt><dd className="font-mono font-semibold text-[var(--admin-success-text)]">+{number(data.economy?.coin?.source)}</dd></div><div className="flex justify-between"><dt>Sink</dt><dd className="font-mono font-semibold text-[var(--admin-danger-text)]">-{number(data.economy?.coin?.sink)}</dd></div><div className="flex justify-between border-t border-[var(--admin-border)] pt-2"><dt className="font-semibold">Net</dt><dd className="font-mono font-semibold">{number(data.economy?.coin?.net)}</dd></div></dl></article></div></AdminCard>
