...
test('registration gate checks status, deadline, and Coin balance', () => {
  ...
  assert.equal(isRegistrationAvailable(tournament, 50, Date.parse('2026-01-01T00:00:00Z')), true);
  assert.equal(isRegistrationAvailable(tournament, 49, Date.parse('2026-01-01T00:00:00Z')), false);
...
test('next match lookup only returns the current participant pending match', () => {
  const tournament = { bracket: { rounds: [{ name: 'Bảng A', matches: [
