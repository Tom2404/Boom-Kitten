...
function publicTournament(value) {
  ...
  const {
    ...
    payoutRequestId, prizePool, ...safe
  } = source;
  return { ...safe, prizePool: { coins: Number(prizePool?.coins) || 0 } };
}
...
async function withdrawPlayer({
  ...
  return withTransaction(async (session) => {
    ...
    const participant = await ParticipantModel.findOneAndUpdate(
      ...
      { $set: { status: 'refunded', paymentStatus: 'refunded', refundRequestId: requestId, refundedAt: now } },
      { new: true, session },
    );
